...
$$
V(s, A)=1 \\ 
V(s, F)=min\{R(s), V(s_1,F)\} , for \space all\space other\space fields \space F
$$
...
$$
V(s, F) = min \{V(s_1,A), V(s_1,B)\},  \space for  \space F=A \space or \space  B \\
V(s, F) = min \{R(s), V(s_1,F)\}, \space  for \space  all \space  fields \space  F \space  other \space  than \space  A \space  or \space  B \space 
$$
...
B(s) = B(s1) \\
R(s) = R(s1) \\
V(s, F) = V(s1, F), \space  for \space  all \space  fields \space  F
$$
...
B(s) = B(s1) + (R(s1) * B(s2)) \\
R(s) = R(s1) * R(s2) \\
V(s, F) = V(s1, F) or V(s2, F), \space  depending \space  on  \space which \space  table \space  F \space  comes \space  from .
$$
...
- parser会在处理SQL的where语句时构建一个谓词，那是会调用方法`conjoinWith()`来将一个当前这个谓词与另外一个谓词取AND运算。
...
simpleDB对应的SQL语句也只支持两种常数：整形和字符串，而在真正的SQL中支持很多数据类型，但是无论有多少数据类型，query是不会具体去处理字段的类型的，而是用一个抽象的Constant类型来描述。考虑这样一个谓词的例子，`MajorId=DId`,这个谓词并不在意数据到底是什么类型的，谓词只关注这些值是否可以有意义地进行比较。
