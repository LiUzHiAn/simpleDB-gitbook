...
现在我们已经了解了如何实现一个索引，我们将注意力转向query planner是如何利用索引这个问题。给定一个SQL查询，planner要执行两个任务：1）必须确定适当的查询树，2）必须为树中的每个运算符选择一个plan。对于第19章我们实现的基本planner而言，这第二个任务是十分trivial的，因为对于每种关系代数操作，它只知道一个实现。例如，它总是使用SelectPlan来实现select操作，而不管索引是否可用。
